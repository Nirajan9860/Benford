...
def calculate_brenford_values(data):
    ...
    frequencies_of_first_digit, total_count = first_digit_frequency(
        list_of_first_digit)
    ...
    for num in range(1, 10):
        ...
        observed_percentage = calculate_percentage(
            observed_frequency, total_count)

        expected_percentage, expected_frequency = calculate_expected_values(
            num, total_count)
...
def test_brenford(distribution):
    ...
    for digit in distribution:
        chi_square = math.pow(
            (digit["observed_frequency"] - digit["expected_frequency"]), 2)
        chi_square_stat += chi_square / digit["expected_frequency"]

    return chi_square < 15.51


def check_brenford(file: str, random_dist: bool = False):
    """
